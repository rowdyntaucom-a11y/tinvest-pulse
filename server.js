...
// DNA WORLD v14.1.5: stable Level 1; history returns through the proven /api/dashboard channel.
...
// The separate history loader is retired. app.js receives history in /api/dashboard again.
html = html.replace(/\s*<script\b[^>]*\bsrc\s*=\s*["'][^"']*\/history-loader-v1191\.js(?:\?[^"']*)?["'][^>]*>\s*<\/script>/gi, '');

const dnaBuild = '1415-dashboard-history';
const dna = '<script src="/dna-world-engine-v141.js?rev='+dnaBuild+'"></script>';
const versionLock = `<script>(function(){const V='v14.1.5';function lock(){document.querySelectorAll('*').forEach(function(el){if(el.children.length)return;const t=el.textContent||'';if(/INVESTOR DNA\\s*·\\s*v\\d+\\.\\d+\\.\\d+/i.test(t))el.textContent=t.replace(/v\\d+\\.\\d+\\.\\d+/i,V);if(/(?:FOUNDATION WORKS|LIGHTS & DETAIL|LIGHTS & LIFE|LIVING MARKET)\\s*·\\s*v\\d+\\.\\d+\\.\\d+/i.test(t))el.textContent=t.replace(/v\\d+\\.\\d+\\.\\d+/i,V);});}lock();setTimeout(lock,250);setTimeout(lock,1000);window.addEventListener('tinvest:dashboard-live',lock);})();</script>`;
html = html.replace('</body>', dna + versionLock + '</body>');
fs.writeFileSync(htmlPath, html);
process.env.TINVEST_BUILD = '14.1.5';
...
// IMPORTANT: do not strip buildPortfolioHistory here. The existing server-core dashboard
// already has the correct history + IMOEX renderer contract. HISTORY_CACHE protects repeats.
