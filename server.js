...
// v14.2.0 — ONE HISTORY ENDPOINT -> ONE OWNER -> ONE CHART RENDERER.
...
const dna = '<script src="/dna-world-engine-v141.js?rev=1420"></script>';
const history = '<script src="/history-chart-v142.js?rev=1420"></script>';
const versionLock = `<script>(function(){const V='v14.2.0';function lock(){document.querySelectorAll('*').forEach(function(el){if(el.children.length)return;const t=el.textContent||'';if(/INVESTOR DNA\\s*·\\s*v\\d+\\.\\d+\\.\\d+/i.test(t))el.textContent=t.replace(/v\\d+\\.\\d+\\.\\d+/i,V);});}lock();setTimeout(lock,250);setTimeout(lock,1000);})();</script>`;
html = html.replace('</body>', history + dna + versionLock + '</body>');
fs.writeFileSync(htmlPath, html);
process.env.TINVEST_BUILD = '14.2.0';
...
core = core.replace(/async function buildPortfolioHistory\(accountId, operations, firstInvestment, portfolioValue\) \{[\s\S]*?\n\}\n\nasync function getMoex\(\) \{/, `async function buildPortfolioHistory(accountId, operations, firstInvestment, portfolioValue) {
  ...
  const cacheKey=String(accountId||'default')+':v142'; const cached=HISTORY_CACHE.get(cacheKey);
  ...
  const moex=await getMoexHistory(from,to),mm=new Map(moex.map(x=>[x.date,x.value])),first=moex.find(x=>x.value>0)?.value||null;for(const p of points){const m=mm.get(p.date);p.imoex=first&&m?Number((m/first*100).toFixed(4)):null}
  const result={available:true,startDate:points[0].date,endDate:points[points.length-1].date,points,valuePoints,investedPoints,method:'isolated_history_v142'};HISTORY_CACHE.set(cacheKey,{createdAt:Date.now(),data:result});return result;
}
