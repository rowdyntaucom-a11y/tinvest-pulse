...
// DNA WORLD v14.1.4: keep Level 1 stable; history owner is bootstrapped before legacy scripts.
...
// Remove any prior injected history owner, then place exactly one owner BEFORE v7106/app.js.
html = html.replace(/\s*<script\b[^>]*\bsrc\s*=\s*["'][^"']*\/history-loader-v1191\.js(?:\?[^"']*)?["'][^>]*>\s*<\/script>/gi, '');
const historyLoader = '<script src="/history-loader-v1191.js?rev=1414-early-owner"></script>';
const firstFrontendScript = /<script\s+src=["']\/v7106\.js[^>]*><\/script>/i;
if (firstFrontendScript.test(html)) html = html.replace(firstFrontendScript, historyLoader + '$&');
else html = html.replace('</body>', historyLoader + '</body>');

const dnaBuild = '1414-early-history-owner';
const dna = '<script src="/dna-world-engine-v141.js?rev='+dnaBuild+'"></script>';
const versionLock = `<script>(function(){const V='v14.1.4';function lock(){document.querySelectorAll('*').forEach(function(el){if(el.children.length)return;const t=el.textContent||'';if(/INVESTOR DNA\\s*·\\s*v\\d+\\.\\d+\\.\\d+/i.test(t))el.textContent=t.replace(/v\\d+\\.\\d+\\.\\d+/i,V);if(/(?:FOUNDATION WORKS|LIGHTS & DETAIL|LIGHTS & LIFE|LIVING MARKET)\\s*·\\s*v\\d+\\.\\d+\\.\\d+/i.test(t))el.textContent=t.replace(/v\\d+\\.\\d+\\.\\d+/i,V);});}lock();setTimeout(lock,250);setTimeout(lock,1000);window.addEventListener('tinvest:dashboard-live',lock);window.addEventListener('tinvest:history-ready',lock);})();</script>`;
html = html.replace('</body>', dna + versionLock + '</body>');
fs.writeFileSync(htmlPath, html);
process.env.TINVEST_BUILD = '14.1.4';
