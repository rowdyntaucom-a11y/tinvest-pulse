...
// DNA WORLD v14.1.3: keep Level 1 renderer stable while history waits for live dashboard readiness.
...
const dnaBuild = '1413-history-live-handoff';
const dna = '<script src="/dna-world-engine-v141.js?rev='+dnaBuild+'"></script>';
const historyLoader = '<script src="/history-loader-v1191.js?rev=1413-live-handoff"></script>';
const versionLock = `<script>(function(){const V='v14.1.3';function lock(){document.querySelectorAll('*').forEach(function(el){if(el.children.length)return;const t=el.textContent||'';if(/INVESTOR DNA\\s*·\\s*v\\d+\\.\\d+\\.\\d+/i.test(t))el.textContent=t.replace(/v\\d+\\.\\d+\\.\\d+/i,V);if(/(?:FOUNDATION WORKS|LIGHTS & DETAIL|LIGHTS & LIFE|LIVING MARKET)\\s*·\\s*v\\d+\\.\\d+\\.\\d+/i.test(t))el.textContent=t.replace(/v\\d+\\.\\d+\\.\\d+/i,V);});}lock();setTimeout(lock,250);setTimeout(lock,1000);window.addEventListener('tinvest:dashboard-live',lock);window.addEventListener('tinvest:history-ready',lock);})();</script>`;
html = html.replace('</body>', dna + historyLoader + versionLock + '</body>');
fs.writeFileSync(htmlPath, html);
process.env.TINVEST_BUILD = '14.1.3';
