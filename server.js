...
// v14.23.0 — DNA CINEMATIC ENVIRONMENT: depth, local light, foreground and parallax; finance/history unchanged.
...
const art='<script src="/dna-art-scene-v148.js?rev=14230"></script>',assets='<script src="/dna-game-assets-v147.js?rev=14230"></script>',dna='<script src="/dna-world-engine-v141.js?rev=14230"></script>',runtime='<script src="/dna-world-runtime-v14180.js?rev=14230"></script>',history='<script src="/history-chart-v142.js?rev=14230"></script>';
const versionLock=`<script>(function(){const V='v14.23.0';function lock(){document.querySelectorAll('*').forEach(function(el){if(el.children.length)return;const t=el.textContent||'';if(/INVESTOR DNA\\s*·\\s*v\\d+\\.\\d+\\.\\d+/i.test(t))el.textContent=t.replace(/v\\d+\\.\\d+\\.\\d+/i,V);});document.querySelectorAll('.dnBadge').forEach(function(b){const t=b.textContent||'';if(/GAME ASSETS|FOUNDATION WORKS|DNA ART|RESOURCES|MINEWORKS|CLEAN ART|ART REBUILD/i.test(t))b.innerHTML='<i></i> CINEMATIC ENV · '+V+' · 11/11';});}lock();setTimeout(lock,250);setTimeout(lock,1000);})();</script>`;
html=html.replace('</body>',history+art+assets+dna+runtime+versionLock+'</body>');fs.writeFileSync(htmlPath,html);process.env.TINVEST_BUILD='14.23.0';
...
app.get('/api/dna-art/status',(req,res)=>{res.setHeader('Cache-Control','no-store');res.json({ok:true,version:'14.23.0',build:'14.23.0',source:'figma-cinematic-environment',path:'/assets/dna-world/l1/figma-pixel-v14190.svg',overlay:'/assets/dna-world/l1/depth-material-v14210.svg'});});
