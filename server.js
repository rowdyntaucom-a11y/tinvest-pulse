...
// DNA WORLD v13.9: one visual owner. Historical renderers are stripped from production.
...
const dnaBuild = '1390-game-art';
const dna = '<script src="/dna-world-engine-v139.js?rev='+dnaBuild+'"></script>';
const historyLoader = '<script src="/history-loader-v1191.js?rev=1192-resilient-history"></script>';
const versionLock = `<script>(function(){const V='v13.9.0';function lock(){document.querySelectorAll('*').forEach(function(el){if(el.children.length)return;const t=el.textContent||'';if(/INVESTOR DNA\\s*·\\s*v\\d+\\.\\d+\\.\\d+/i.test(t))el.textContent=t.replace(/v\\d+\\.\\d+\\.\\d+/i,V);if(/(?:FOUNDATION WORKS|LIGHTS & DETAIL|LIGHTS & LIFE|LIVING MARKET)\\s*·\\s*v\\d+\\.\\d+\\.\\d+/i.test(t))el.textContent=t.replace(/v\\d+\\.\\d+\\.\\d+/i,V);});}lock();setTimeout(lock,250);setTimeout(lock,1000);window.addEventListener('tinvest:dashboard-live',lock);window.addEventListener('tinvest:history-ready',lock);})();</script>`;
html = html.replace('</body>', dna + historyLoader + versionLock + '</body>');
fs.writeFileSync(htmlPath, html);
process.env.TINVEST_BUILD = '13.9.0';
