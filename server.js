...
const versionLock=`<script>(function(){const V='v15.1.0';function lock(){document.querySelectorAll('*').forEach(function(el){if(el.children.length)return;const t=el.textContent||'';if(/INVESTOR DNA\\s*·\\s*v\\d+\\.\\d+\\.\\d+/i.test(t))el.textContent=t.replace(/v\\d+\\.\\d+\\.\\d+/i,V);});document.querySelectorAll('.dnBadge').forEach(function(b){const t=b.textContent||'';if(/GAME ASSETS|FOUNDATION WORKS|DNA ART|RESOURCES|MINEWORKS|CLEAN ART|ART REBUILD|DETAIL READABILITY|CINEMATIC ENV|CLEAN POLISH|CHARACTER MATERIAL|WORLD COHESION|MOTION PASS|WORLD SCALE|LEVEL WORLDS|WORLD MASTER|CINEMATIC WORLD/i.test(t))b.innerHTML='<i></i> CINEMATIC WORLD · '+V+' · 11/11';});}lock();setTimeout(lock,250);setTimeout(lock,1000);})();</script>`;
html=html.replace('</body>',history+art+assets+dna+runtime+preview+levelworld+master+cinematic+versionLock+'</body>');fs.writeFileSync(htmlPath,html);process.env.TINVEST_BUILD='15.1.0';
...
app.get('/api/dna-art/status',(req,res)=>{res.setHeader('Cache-Control','no-store');res.json({ok:true,version:'15.1.0',build:'15.1.0',source:'cinematic-world',path:'/assets/dna-world/l1/figma-pixel-v14190.svg',overlay:'/assets/dna-world/l1/depth-material-v14210.svg'});});
