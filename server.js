...
// v14.13.0 — ART DEPTH PASS: richer local DNA world art and larger readable activity; finance/history unchanged.
...
const art='<script src="/dna-art-scene-v148.js?rev=14130"></script>',assets='<script src="/dna-game-assets-v147.js?rev=14130"></script>',dna='<script src="/dna-world-engine-v141.js?rev=14130"></script>',history='<script src="/history-chart-v142.js?rev=14130"></script>';
const versionLock=`<script>(function(){const V='v14.13.0';function lock(){document.querySelectorAll('*').forEach(function(el){if(el.children.length)return;const t=el.textContent||'';if(/INVESTOR DNA\\s*·\\s*v\\d+\\.\\d+\\.\\d+/i.test(t))el.textContent=t.replace(/v\\d+\\.\\d+\\.\\d+/i,V);});document.querySelectorAll('.dnBadge').forEach(function(b){const t=b.textContent||'';if(/GAME ASSETS|FOUNDATION WORKS|DNA ART|RESOURCES|MINEWORKS|CLEAN ART|ART DEPTH/i.test(t))b.innerHTML='<i></i> ART DEPTH · '+V+' · 4/11';});}lock();setTimeout(lock,250);setTimeout(lock,1000);})();</script>`;
html=html.replace('</body>',history+art+assets+dna+versionLock+'</body>');fs.writeFileSync(htmlPath,html);process.env.TINVEST_BUILD='14.13.0';
...
function fetchDnaArtUrl(url,hops=0){
 return new Promise((resolve,reject)=>{
  let settled=false;
  const req=https.get(url,{headers:{'User-Agent':'Mozilla/5.0 TInvestPulse/14.13.0','Accept':'image/avif,image/webp,image/*,*/*;q=0.8'}},r=>{
...
app.get('/api/dna-art/status',(req,res)=>{res.setHeader('Cache-Control','no-store');res.json({ok:true,version:'14.13.0',build:'14.13.0',source:'local-original-depth',path:'/assets/dna-world/l1/scene-base-clean.svg'});});
