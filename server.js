...
// v14.24.0 — DNA DETAIL READABILITY: brighter underground, cleaner material detail and contrast; finance/history unchanged.
...
const art='<script src="/dna-art-scene-v148.js?rev=14240"></script>',assets='<script src="/dna-game-assets-v147.js?rev=14240"></script>',dna='<script src="/dna-world-engine-v141.js?rev=14240"></script>',runtime='<script src="/dna-world-runtime-v14180.js?rev=14240"></script>',history='<script src="/history-chart-v142.js?rev=14240"></script>';
const versionLock=`<script>(function(){const V='v14.24.0';function lock(){document.querySelectorAll('*').forEach(function(el){if(el.children.length)return;const t=el.textContent||'';if(/INVESTOR DNA\\s*·\\s*v\\d+\\.\\d+\\.\\d+/i.test(t))el.textContent=t.replace(/v\\d+\\.\\d+\\.\\d+/i,V);});document.querySelectorAll('.dnBadge').forEach(function(b){const t=b.textContent||'';if(/GAME ASSETS|FOUNDATION WORKS|DNA ART|RESOURCES|MINEWORKS|CLEAN ART|ART REBUILD/i.test(t))b.innerHTML='<i></i> DETAIL READABILITY · '+V+' · 11/11';});}lock();setTimeout(lock,250);setTimeout(lock,1000);})();</script>`;
html=html.replace('</body>',history+art+assets+dna+runtime+versionLock+'</body>');fs.writeFileSync(htmlPath,html);process.env.TINVEST_BUILD='14.24.0';
...
app.get('/api/dna-art/status',(req,res)=>{res.setHeader('Cache-Control','no-store');res.json({ok:true,version:'14.24.0',build:'14.24.0',source:'figma-detail-readability',path:'/assets/dna-world/l1/figma-pixel-v14190.svg',overlay:'/assets/dna-world/l1/depth-material-v14210.svg'});});
