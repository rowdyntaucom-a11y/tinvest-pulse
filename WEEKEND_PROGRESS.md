...
### 12:02 MSK autonomous pass — Bond → Income linkage regression lock and audit sync
- Re-read `PROJECT_CONTEXT.md`, `v2/PRODUCT_STRUCTURE_V1.md`, `ADVANCED_INVESTOR_AUDIT.md`, `QVANIX_TERMINAL_DECISIONS.md`, `LEGAL_REVIEW_2026-09-11.md` and this progress log before changes.
- Verified the already-landed transaction-marker UI/backend boundary and the existing Bond → Income schedule linkage instead of duplicating them. The audit was stale: both still appeared in ACTIVE even though production code already implements their safe scope.
- Quant methodology review found a boundary-hardening gap in `buildBondIncomeLinkage`: the current `/api/payouts.events` contract is forecast-only and stores realized FACT rows separately in `actual.items`, so production does not currently mix them; however the pure linkage function itself did not reject a future mixed caller.
- PR #125 adds `BOND_INCOME_LINKAGE_CALC_VERSION = 1.1` and explicitly rejects `status=FACT` before grouping coupon schedule events. FIGI-only matching, current positive-value bond eligibility, forecast arithmetic and UI remain unchanged.
- Added `v2/tests/bondIncomeLinkage.test.ts` and wired it into `test:core`. Regression coverage locks case-insensitive FIGI matching, COUPON-only filtering, exact next-coupon ordering, linkage/value coverage, scheduled gross totals, FACT rejection, invalid dates and invalid/non-positive position values.
- Code-quality pass: GitHub `v2 build` run #178 completed successfully. `npm run build`, full `npm run test:core`, and syntax checks for `payouts-core.js`, `server-core.js`, `production-v158.js`, `production-v159.js` and `production-v160.js` all passed.
- Mobile-UX pass: no UI/CSS/layout change; existing compact BOND → INCOME line and Samsung/Android density remain unchanged.
- Release pass: runtime diff is one pure Income boundary plus regression test and test-runner registration; no broker/backend route, permissions, credentials, legal/payment text, DNA renderer or trade behavior changed. PR #125 was squash-merged to `main` as `fd76f617477922c31e1bd7b90d32bc5d0aa6e7e7`.
- `ADVANCED_INVESTOR_AUDIT.md` was refreshed: transaction markers and safe Bond → Income schedule linkage are now DONE; exact realized-operation ↔ scheduled-coupon reconciliation remains GATED until a shared verified coupon-event identity exists. No ticker/date/amount proximity matching is approved.
...
### 16:04 MSK autonomous pass — bond metadata coverage context
- Re-read `PROJECT_CONTEXT.md`, `v2/PRODUCT_STRUCTURE_V1.md`, `ADVANCED_INVESTOR_AUDIT.md`, `QVANIX_TERMINAL_DECISIONS.md`, `LEGAL_REVIEW_2026-09-11.md` and this progress log before changes, then reconciled the requested queue with current `main`.
- Chose the issuer/sector/country data-quality gap rather than adding another analytics widget or duplicating already-landed Income work. Existing bond concentration formulas and broker metadata normalization remain unchanged.
- PR #147 updates only `v2/src/features/portfolio/BondAnalytics.tsx` and was squash-merged as `4d8f4be5363acd2a7db152a931fb1e6d66daa2ef`.
- Partial issuer/sector/country metadata coverage is now explicitly marked `частично`; unavailable dimensions retain the verified coverage percentage instead of showing an unqualified metadata absence. Exact covered RUB value versus the current bond sleeve is available in the native tooltip, while top-category share and `Nₑ` remain shares/statistics of the covered subset only.
- Quant methodology pass: no denominator, HHI/effective-count formula, market value, forecast, return assumption or recommendation changed; the pass only makes the existing coverage denominator visible so partial broker metadata cannot masquerade as complete coverage.
- Code-quality pass: pre-merge `v2 build` run #221 succeeded, including `npm ci`, dependency-security gates, TypeScript/Vite build, full `test:core`, and runtime syntax checks for `payouts-core.js`, `server-core.js`, `production-v158.js`, `production-v159.js` and `production-v160.js`. Post-merge main `v2 build` run #223 also completed successfully.
- Mobile-UX pass: no new widget, CSS section or scroll region was added. The existing compact one-line bond-risk row is reused; detailed RUB coverage remains in tooltip/title so Samsung/Android density is not expanded into another card.
- Release pass: one-file UI-context diff, no backend/broker route, credential, trade behavior, DNA renderer, legal/payment wording or deployment configuration. Rollback is the single squash commit if a display regression is later attributed to it.
- Several stale historical push workflows still fail on ordinary `main` pushes, but the scoped `v2 build` production gate for the accepted SHA is green; those unrelated failures are not treated as a regression of this change.
- Render auto-deploy was not manually triggered. Read-only monitoring was attempted after merge, but the connector returned `no workspace selected` and explicitly requires user confirmation before selecting one; deployment state for `tinvest-pulse-v2-preview` and `tinvest-pulse` is therefore not guessed.
- Legal publication blocker remains unchanged; no RU/EN offer, privacy-policy or consent wording was published.
- Next safe focus: continue compact live/mobile data-quality validation for Allocation Lab and transaction-marker density, or surface position-income observation-window provenance only if it can fit the existing inspector without adding a duplicate widget or breaking Samsung one-screen density.

### 17:07 MSK autonomous pass — strategy-scenario validation regression lock
- Re-read the mandatory project/product/audit/terminal/legal/progress documents and started from `main` commit `e99fe94839a40d0097582c24d27a853b187d1fd7`, which already included the separate asset-history market-value ranking fix; no parallel work was overwritten.
- The existing explicit strategy comparison boundary already enforced user-authored equity/bond scenarios, exact 100% weights, no normalization, no expected-return ranking and no personalized buy/sell output. This pass hardened that policy rather than adding another Analytics widget.
- PR #151 extracts only the scenario acceptance policy into `strategyScenarioPolicy.ts` and adds it to mandatory `test:core`: both equity and bond must be present with positive finite weights summing to 100%, tolerances must be finite/non-negative, IDs must be unique/non-empty, and only the first four explicit scenarios are accepted deterministically.
- Quant methodology pass: strategy candidates remain entirely user-supplied; no optimizer, expected return, ranking, instrument orders or invented market assumptions were added. Existing drift/rebalance arithmetic and class-level turnover formulas were not changed.
- Code-quality pass deliberately caught two integration issues before merge. Run #227 failed only because direct Node strip-types cannot resolve the production module's extensionless bundler imports. A first attempted `.ts` import correction was rejected by TypeScript in run #228 (`TS5097`) and was not merged. The accepted solution leaves production bundler imports intact and tests a dependency-free extracted validation boundary instead.
- Final pre-merge GitHub `v2 build` run #231 completed successfully: `npm ci`, both dependency-security gates, TypeScript/Vite build, full `test:core`, asset-history regression, and syntax checks `payouts-core.js`, `server-core.js`, `production-v158.js`, `production-v159.js` and `production-v160.js` all passed.
- Mobile-UX pass: no UI/CSS/layout/rendering changes; Samsung/Android density and no-duplication rules are unchanged.
- Release pass: final diff is the extracted pure validation policy, its regression test, one import/use replacement in the existing comparison primitive, and `test:core` registration. PR #151 was squash-merged to `main` as `400aa410f6a87b99b7185574f34a11298e6c3a5d`; no broker/backend route, credential, legal/payment wording, DNA renderer or trading execution behavior changed.
...
### 23:24 MSK autonomous pass — deterministic Portfolio Top-N constituent context
- Re-read the mandatory project/product/audit/terminal/legal/progress documents and reconciled them with the actual current `main` before changing code.
- Started the Portfolio exposure depth pass from `main` `1d6cbf87f9bbc15524827031abb2f20e7f852b37`, then detected that `main` advanced independently to `3a6b603dbaed793053857dfe39f0bc9f51c92183` with a separate Allocation Lab mobile-legibility change. Stale PR #169 was closed unmerged, and the change was recreated from the new head rather than overwriting parallel work.
- PR #170 bumps `PORTFOLIO_EXPOSURE_DIAGNOSTICS_VERSION` to `1.1` and extends the already-reviewed Top-N calculation with deterministic constituent identity: `topPositions` carries the ticker/name label plus the exact current weight used in the aggregate; `identifiedIncludedCount` keeps unknown identity explicit instead of inventing labels.
- The Top-N weight formula is unchanged. Invalid/non-positive weights remain excluded, sorting remains weight-descending, and exact-weight ties now use deterministic label ordering so the same economic input cannot produce a different constituent list merely because upstream array order changed.
- Regression coverage locks input-order invariance, equal-weight tie behavior, invalid-weight rejection, ticker→name fallback, unidentified positions, invalid requested count and empty input behavior.
- Quant methodology pass: no return, P/L, attribution, concentration denominator, forecast or recommendation formula changed; this pass only makes the already-calculated current-weight exposure explainable by constituent.
- Code-quality pass: pre-merge GitHub `v2 build` run #268 completed successfully on head `32ecf9aaa5141eb4ce4e31a538a88587c89bb385`, including the mandatory core regression suite and existing build/security gates.
- Mobile-UX pass: no React/CSS/layout change in PR #170; Samsung/Android density and no-duplication behavior are unchanged. Constituent output remains calculation groundwork for a later compact existing-row integration rather than a new widget.
- Release pass: PR #170 changed only the pure exposure boundary and its regression test and was squash-merged as `56f1bbe33ccfc762d57750174a07b539865f137b`. No backend/broker route, credential, legal/payment wording, DNA renderer or trade behavior changed.
- After that merge, `main` advanced independently again to `32a3cdf9156e94dea55222c5ce41e0089d917494` with a separate Income mobile-legibility pass that is a direct descendant of `56f1bbe…`; no conflict or rollback is indicated.
- Several stale historical push workflows continue to fail independently of scoped v2 changes. The reviewed PR gate for this change is green; no regression attributable to PR #170 was identified.
- Render auto-deploy was not manually triggered. Read-only monitoring remains blocked because no user-confirmed Render workspace is selected; connector rules explicitly prohibit autonomous workspace selection, so `tinvest-pulse-v2-preview` / `tinvest-pulse` state is not guessed.
- Legal publication blocker remains unchanged; no RU/EN offer, privacy-policy or consent wording was published.
- Next safe focus: surface `topPositions` only inside the existing Portfolio structure methodology row/tooltip if that passes Samsung density review; otherwise continue deliberate live-data-quality/dependency review without adding another widget.

## 2026-09-13

### 04:10 MSK documentation catch-up — analytics history-integrity passes
- Re-read the mandatory project/product/audit/terminal/legal/progress documents before the current production pass and reconciled the advanced-analytics queue against current `main`.
- PR #175 / `af0fce72cb50127f5b38c6ea68b20b60554db56d`: Monte Carlo bootstrap history integrity was hardened. Exact duplicate dates collapse deterministically; conflicting TWR values on one date fail closed; accepted sample boundaries are explicit. The historical block-bootstrap methodology and short-history gates were not changed. Pre-merge `v2 build` #273 and post-merge `main` #274 succeeded.
- PR #176 / `ed9808e586005baff7b1520bdbfc08718bd115a3`: historical VaR/CVaR history integrity was hardened with the same exact-duplicate collapse / conflicting-date fail-closed policy and sample provenance. VaR/CVaR methodology and 126/252 observation gates were not changed. Pre-merge `v2 build` #275 and post-merge `main` #276 succeeded.
- PR #177 / `878b3e36936191c5189a2524d3ceb21642fa8d5d`: per-asset correlation history now rejects conflicting same-date prices before return-interval construction; exact duplicates collapse deterministically and series provenance is explicit. Pearson correlation remains based on paired return intervals with the existing 60/252 gates. Pre-merge `v2 build` #277 succeeded.
- PR #178 / `f9b0c923f15c5ced5e10953df4bced566851ccef`: benchmark-relative analytics now reject conflicting duplicate Portfolio/IMOEX dates and expose sample provenance. Tracking Error, Information Ratio, Beta and correlation formulas and 60/252 gates remain unchanged. Pre-merge `v2 build` #279 succeeded.
- PR #179 / `78e0b5534225748bacfc41fd9ee08e8206e58c0a`: rolling return/volatility/drawdown integrity was hardened. Conflicting duplicate TWR dates block rolling metrics; benchmark conflicts are tracked separately; benchmark/excess return is published only when every adjacent return interval in the selected 20/60/120/252 window has valid IMOEX endpoints. Missing benchmark data no longer gets bridged by filtering rows out, while valid portfolio-only rolling metrics remain available.
- Quant methodology pass for #179: no return, volatility, drawdown or annualization formula changed. The change only prevents ambiguous dates and incomplete benchmark windows from being presented as valid common-sample comparisons.
- Code-quality pass for #179: initial PR `v2 build` #282 passed TypeScript/Vite but failed `test:core` because the existing regression still asserted rolling calculation version `1.0` and the old uninformative benchmark-pair count. Production was not merged. The regression was updated to `1.1` and truthful adjacent-pair coverage; repeat `v2 build` #283 then passed `npm ci`, both dependency-security gates, TypeScript/Vite, full `test:core`, asset-history regression and all current runtime syntax checks.
- Mobile-UX pass for #175–#179: no new React widget, CSS block, navigation item or scroll region was introduced; Samsung/Android density and duplication rules are unchanged.
- Release pass for #179: runtime change is limited to the pure rolling-risk boundary plus deterministic tests/test registration; no backend/broker route, credential, legal/payment wording, DNA renderer or trading behavior changed. `main` is production-worthy at `78e0b5534225748bacfc41fd9ee08e8206e58c0a`.
- Render auto-deploy was not manually triggered. Read-only monitoring after #179 remains blocked because no user-confirmed Render workspace is selected; connector rules prohibit autonomous workspace selection, so status of `tinvest-pulse-v2-preview` / `tinvest-pulse` is not guessed.
