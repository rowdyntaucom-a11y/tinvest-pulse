...
- Income Sources: transparent 12M YoC added; payout-growth metric gated until two comparable annual periods exists.
...
- Several stale historical patch workflows still fail on ordinary `main` pushes, but the scoped `v2 build` production gate for the accepted SHA is green; those unrelated failures are not treated as a regression of this change.
...
- Final pre-merge GitHub `v2 build` run #231 completed successfully: `npm ci`, both dependency-security gates, TypeScript/Vite build, full `test:core`, asset-history regression, and syntax checks for `payouts-core.js`, `server-core.js`, `production-v158.js`, `production-v159.js` and `production-v160.js` all passed.
