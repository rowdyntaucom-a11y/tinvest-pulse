...
Financial dashboard remains healthy and the served production wrapper is now **v14.7.0 GAME ASSET PIPELINE foundation**. The existing `dna-world-engine-v141.js` remains the single production renderer and keeps the working animated Level 1 scene. A new asset registry, `public/dna-game-assets-v147.js`, is injected before the renderer. It defines named asset slots for background, mine, buildings, workers, logistics, resources and FX; supports registration, preload, cache, draw and status; and is intentionally safe when no external assets are registered. This creates the bridge from procedural Canvas art to real PNG/WebP/SVG sprite assets without changing finance/history behavior or introducing a second world renderer.

Figma is connected but still reports Starter/View at the time of this update; the user will notify when a paid/edit-capable seat activates. Ideogram MCP has been connected by the user in ChatGPT and its actions are visible in settings, but the current chat runtime has not exposed Ideogram as a callable tool. Project development must not wait for either service.
...
Implemented/established: live T-Bank data; portfolio positions/capital; passive-income calculations; dashboard analytics; independent history/IMOEX loading architecture; INTEL engines; PULSE presentation; Investor DNA concept; animated Level 1; 11-level capital model; single-renderer architecture; real-time day/night; market-weather; environment/lights/detail layers; stable production-owned version labels; mine→cart→delivery animation loop; and the first external game-asset registry/pipeline foundation in v14.7.0.

Remaining priorities: produce reference-quality Level 1 art assets; map them into the v14.7 registry; replace procedural primitives asset-by-asset while preserving fallback safety; richer original characters and animation states; clearer load/unload/build beats; progressive construction tied continuously to capital; stronger background depth and props; distinct evolution across 11 levels; deeper Portfolio DNA diagnostics; analytics validation; mobile performance.
...
Keep v14.7.0 as a non-breaking bridge and begin the actual **GAME ASSET PASS** for Level 1 `ФУНДАМЕНТ`: generate/select the first cohesive asset family, starting with background depth + mine + one worker + loaded cart + construction module, register those files in `dna-game-assets-v147.js`, then make the existing single renderer draw them with Canvas fallbacks. Validate on the user's Samsung/Android device before replacing the next asset group. Do not wait for Figma or Ideogram availability; reconnect those tools later when they become usable.