...
Continue from `progress/2026-09-25-samurai-income-calendar-v2.md` + current `main`. Samsung recording 1000031734 confirmed the latest Samurai UI fixes and Report / Category / Currency breadth are live. Income now includes selectable 3M/6M/12M forward payout windows, nearest payout detail, and an explicitly separate fail-closed market dividend-discovery chapter. Next active functional surface: **rebalancing workflow UI**, then Portfolio Lab, technical discovery and screener. Samurai remains the sole Snowball+ reference shell; DNA WORLD stays frozen.
