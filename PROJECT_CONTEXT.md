...
Continue from `progress/2026-09-25-samurai-bond-yield-v1.md` + current `main`. Samsung recording 1000031745 confirmed the inline Screener remains stable on the real device with no return-to-top regression. Samurai Assets > Bonds now also has verified YTM / modified-duration / maturity / ACI / rate-shock depth backed by T-Bank BondBy, GetBondCoupons and GetMarketValues yield; missing complex values remain explicit gaps. Next active surface: **verified equity fundamentals depth**, then broad-market dividend discovery and remaining Atlas-to-functional navigation.
