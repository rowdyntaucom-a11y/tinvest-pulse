...
Continue from `progress/2026-09-25-samurai-report-category-currency-v1.md` + current `main`. Samsung recording 1000031733 confirmed the latest real-device hierarchy fixes are live, so Samurai moved back to functional breadth. Report + Categories + Currencies are now implemented as chapters 08–10 with broker-P/L/cost-basis reconciliation and explicit unknown-currency handling. Samurai remains the sole Snowball+ reference shell. Next active functional surface: **expanded payout/calendar UX + separation of portfolio payouts from broad-market dividend discovery**.
