...
Continue from `progress/2026-09-25-samurai-portfolio-lab-v1.md` + current `main`. Samsung recording 1000031736 showed no new blocking real-device regression; it still reflected the pre-Rebalancing Analysis Atlas while Income Calendar v2 was already live. Samurai now includes Rebalancing v1 plus Portfolio Laboratory v1 with user-authored scenario comparison and historical MCFTR/RGBITR class simulations on a common sample. Next active functional surface: **technical discovery / fallen-assets tooling**, then user-facing screener. Samurai remains the sole Snowball+ reference shell; DNA WORLD stays frozen.
