...
Continue from `progress/2026-09-25-samurai-recording-1730-cascade.md` + current `main`. Samsung recording 1000031730 exposed and fixed the residual Atlas cascade: Assets/Income titles no longer collapse into a narrow legacy header column, the “НИЖЕ” cue no longer overlays the final chapter, and Home compact source verification no longer reserves a mostly empty viewport. Samurai remains the only active Snowball+ reference shell. Next active functional surface: **Report + category + currency drill-down**.
