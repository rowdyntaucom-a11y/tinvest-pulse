...
// For ALL, one known false condition is enough to establish NO_MATCH even when another value is missing.
...
// For ANY, one known true condition is enough to establish MATCH.
assert.equal(result.status, 'MATCH')
assert.equal(result.matched, true)
...
// Crossing rules inherit strict chronological validation from the user-authored alert boundary.
result = evaluateUserScreener({
  id: 'crossing',
  mode: 'ALL',
...
// Config validation fails closed: duplicate ids, empty sets, too many rules and malformed mode are rejected.
result = evaluateUserScreener({
  id: 'duplicate',
  mode: 'ALL',
  rules: [
...
result = evaluateUserScreener({ id: 'bad-mode', mode: 'ALL', rules: allMatch.rules, } as UserScreener, current, previous)
// Sanity check valid cast remains valid; malformed runtime mode is tested separately.
assert.equal(result.status, 'MATCH')
