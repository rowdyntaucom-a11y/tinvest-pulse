...
dirty.push({ date: day(10), portfolio: 100, imoex: null })
dirty.push({ date: day(10), portfolio: 101, imoex: null })
...
assert.equal(cleaned.available, false)
assert.equal(cleaned.historyPoints, 60)
assert.equal(cleaned.returnObservations, 59)
assert.match(cleaned.reason ?? '', /59/)
