...
for (const generatedAt of [null, '2026-09-15T11:59:00.000Z']) {
  assert.equal(evaluatePayoutTrust({ available: true, stale: true, generatedAt, eligibleAssets: 4, resolvedAssets: 4, coverageRatio: 1, scheduleComplete: true }, now).status, 'STALE')
}
const payoutPartial = evaluatePayoutTrust({ available: true, stale: false, generatedAt: null, eligibleAssets: 4, resolvedAssets: 3, coverageRatio: .75, scheduleComplete: false }, now)
assert.equal(payoutPartial.status, 'PARTIAL')
assert.equal(payoutPartial.safeToCalculate, false)
const verifiedEmptyPayouts = evaluatePayoutTrust({ available: true, stale: false, generatedAt: null, eligibleAssets: 0, resolvedAssets: 0, coverageRatio: 1, scheduleComplete: true }, now)
assert.equal(verifiedEmptyPayouts.status, 'LIVE', 'a verified empty payout universe is complete')
assert.equal(verifiedEmptyPayouts.coverage, 'COMPLETE')
assert.equal(verifiedEmptyPayouts.safeToCalculate, true)
assert.equal(evaluatePayoutTrust({ loading: true, available: false, stale: false, generatedAt: null, eligibleAssets: 0, resolvedAssets: 0, coverageRatio: 0, scheduleComplete: false }, now).status, 'LOADING')
assert.equal(evaluatePayoutTrust({ available: false, stale: false, generatedAt: null, eligibleAssets: 0, resolvedAssets: 0, coverageRatio: 0, scheduleComplete: false }, now).status, 'UNAVAILABLE')
