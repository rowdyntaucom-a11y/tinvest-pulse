...
const staleFallback = evaluateDataTrust({ ...base, sourceState: 'FALLBACK', explicitStale: true, fetchedAt: null })
assert.equal(staleFallback.status, 'FALLBACK', 'fallback provenance takes precedence over stale status')
assert.equal(staleFallback.freshness, 'STALE', 'fallback freshness remains explicit rather than being reported as fresh')
...
const agingHistoryRows = [
  { date: '2026-09-15', portfolio: 1, imoex: 1 },
  { date: '2026-09-14', portfolio: 0.99, imoex: 0.98 },
]
const historyFreshAt = Date.parse('2026-09-15T00:00:00.000Z')
assert.equal(evaluateHistoryTrust(agingHistoryRows, historyFreshAt).portfolioTrust.status, 'LIVE')
assert.equal(
  evaluateHistoryTrust(agingHistoryRows, historyFreshAt + 7 * 86_400_000 + 1).portfolioTrust.status,
  'STALE',
  'the same history must age without requiring a new observation row',
)
...
assert.equal(evaluateHistoryTrust([{ date: '2026-99-99', portfolio: 1, imoex: 1 }], now).trust.status, 'UNAVAILABLE')
...
const payoutLive = evaluatePayoutTrust({ available: true, stale: false, generatedAt: '2026-09-15T11:59:00.000Z', eligibleAssets: 4, resolvedAssets: 4, scheduleComplete: true }, now)
assert.equal(payoutLive.status, 'LIVE')
assert.equal(payoutLive.safeToCalculate, true, 'a complete live calendar may expose the next payout')
...
for (const blockedPayout of [
  evaluatePayoutTrust({ available: true, stale: true, generatedAt: '2026-09-15T11:59:00.000Z', eligibleAssets: 4, resolvedAssets: 4, scheduleComplete: true }, now),
  payoutPartial,
  evaluatePayoutTrust({ loading: true, available: false, stale: false, generatedAt: null, eligibleAssets: 0, resolvedAssets: 0, scheduleComplete: false }, now),
  evaluatePayoutTrust({ available: false, stale: false, generatedAt: null, eligibleAssets: 0, resolvedAssets: 0, scheduleComplete: false }, now),
]) assert.equal(blockedPayout.safeToCalculate, false, `${blockedPayout.status} must hide future-derived payouts`)
