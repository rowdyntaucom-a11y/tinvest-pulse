...
assert.match(interactionCss, /body:has\(\.qv-personalize\[open\]\) \.context-help\s*\{[^}]*visibility:\s*hidden[^}]*pointer-events:\s*none/s)
...
assert.match(helpSource, /aria-label="Закрыть подсказку"/)
assert.match(helpCss, /max-height:min\(440px,48dvh\)/)
assert.match(helpCss, /bottom:calc\(138px \+ env\(safe-area-inset-bottom\)\)/)
