...
// Board must consume the same split boundary: raw calendar.next cannot bypass
// payout eligibility, and calendar.available cannot erase the observed ledger.
const board = fs.readFileSync(new URL('../src/features/board/QvanixBoard.tsx', import.meta.url), 'utf8')
assert.match(board, /separateTrustedIncomeData\(calendar, payoutTrust\?\.safeToCalculate === true\)/)
assert.match(board, /const next = trustedIncome\?\.taxCalendar\.next \?\? null/)
assert.doesNotMatch(board, /const next = calendar\?\.next \?\? null/)
assert.match(board, /calendar\?\.actual\?\.observation\?\.available === true && \(trustedIncome\?\.actualEvents\.length \?\? 0\) > 0/)
