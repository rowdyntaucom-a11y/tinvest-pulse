...
const exactDuplicateA = makeSeries('EXACT_A', pattern(60, [0.01, -0.004, 0.006]))
const exactDuplicateB = makeSeries('EXACT_B', pattern(60, [-0.003, 0.005, -0.001]))
exactDuplicateA.points.splice(20, 0, { ...exactDuplicateA.points[19] })
const exactDuplicate = calculateAllocationDiagnostics([exactDuplicateA, exactDuplicateB])
...
const conflictingA = makeSeries('CONFLICT_A', pattern(60, [0.01, -0.004, 0.006]))
const conflictingB = makeSeries('CONFLICT_B', pattern(60, [-0.003, 0.005, -0.001]))
conflictingA.points.splice(20, 0, {
  date: conflictingA.points[19].date,
  value: conflictingA.points[19].value * 1.01,
})
const conflicting = calculateAllocationDiagnostics([conflictingA, conflictingB])
...
assert.equal(conflicting.from, null)
assert.equal(conflicting.to, null)
assert.equal(conflicting.equalWeight, null)
assert.equal(conflicting.minimumVariance, null)
assert.equal(conflicting.equalRiskContribution, null)
assert.match(conflicting.note, /конфликтующие значения/)
