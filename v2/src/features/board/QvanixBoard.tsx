...
export function QvanixBoard({ snapshot, analytics, xirrPercent, metricEligibility, pinnedModules, onNavigate }: Props) {
  ...
  const payoutTrust = calendar
    ? evaluatePayoutTrust({ loading: incomeLoading, available: calendar.available, stale: calendar.stale, generatedAt: calendar.generatedAt, eligibleAssets: calendar.coverage.eligibleAssets, resolvedAssets: calendar.coverage.resolvedAssets, scheduleComplete: calendar.integrity.complete }, Date.now())
    : null
