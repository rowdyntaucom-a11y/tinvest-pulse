...
import { separateTrustedIncomeData } from '../income/incomeDataTrust'
...
export function QvanixBoard({ snapshot, analytics, xirrPercent, metricEligibility, pinnedModules, onNavigate }: Props) {
  ...
  const payoutTrust = calendar
    ? evaluatePayoutTrust({ loading: incomeLoading, available: calendar.available, stale: calendar.stale, generatedAt: calendar.generatedAt, eligibleAssets: calendar.coverage.eligibleAssets, resolvedAssets: calendar.coverage.resolvedAssets, coverageRatio: calendar.coverage.coverageRatio, scheduleComplete: calendar.integrity.complete }, Date.now())
    : null
  const trustedIncome = calendar
    ? separateTrustedIncomeData(calendar, payoutTrust?.safeToCalculate === true)
    : null

  const modules = useMemo(() => {
    const next = trustedIncome?.taxCalendar.next ?? null
    const nextDate = safeDate(next?.date)
    const nextAmount = next?.gross != null && Number.isFinite(next.gross) ? next.gross : null
    const actualAvailable = calendar?.actual?.observation?.available === true && (trustedIncome?.actualEvents.length ?? 0) > 0
  ...
  }, [analytics, calendar, incomeLoading, metricEligibility, pinnedModules, snapshot, trustedIncome, xirrPercent])
