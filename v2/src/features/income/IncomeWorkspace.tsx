...
export function IncomeWorkspace({ passiveIncome, averageMonthlyPassiveIncome, startDate, positions, onOpenAsset }: Props) {
  ...
  const payoutTrust = evaluatePayoutTrust({ loading, available: data.available, stale: data.stale, generatedAt: data.generatedAt, eligibleAssets: data.coverage.eligibleAssets, resolvedAssets: data.coverage.resolvedAssets, coverageRatio: data.coverage.coverageRatio, scheduleComplete: data.integrity.complete }, Date.now())
