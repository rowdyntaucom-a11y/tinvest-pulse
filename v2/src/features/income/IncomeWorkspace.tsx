...
export function IncomeWorkspace({ trustNow, passiveIncome, averageMonthlyPassiveIncome, startDate, positions, onOpenAsset }: Props) {
  ...
  const payoutTrust = evaluatePayoutTrust({ loading, available: data.available, stale: data.stale, generatedAt: data.generatedAt, eligibleAssets: data.coverage.eligibleAssets, resolvedAssets: data.coverage.resolvedAssets, scheduleComplete: data.integrity.complete }, trustNow)
  // Historical facts remain available; every future-derived surface receives an empty schedule unless eligibility is proven.
  const trustedData = useMemo<PayoutCalendar>(() => payoutTrust.safeToCalculate ? data : ({ ...data, forecast: { gross: 0, tax: 0, net: 0, count: 0 }, next: null, months: [], events: [] }), [data, payoutTrust.safeToCalculate])

  const calendarMonths = useMemo(
    () => buildIncomeCalendarVisual(trustedData.events, trustedData.period.from, 12),
    [trustedData.events, trustedData.period.from],
  )
  const filteredCalendarEvents = useMemo(
    () => filterIncomeCalendarEvents(trustedData.events, calendarMonth),
    [trustedData.events, calendarMonth],
  )
  ...
  const bondIncomeLinkage = useMemo(
    () => buildBondIncomeLinkage(positions, trustedData.events),
    [positions, trustedData.events],
  )

  const allSourceRows = useMemo(
    () => buildIncomeSourceRows(data.actual.items, trustedData.events, positions, Number.MAX_SAFE_INTEGER),
    [data.actual.items, trustedData.events, positions],
  )
    ...
    <div className="income-workspace">
      ...
      {view === 'taxes' && (
        <Suspense fallback={<section className="panel"><div className="income-empty">Налоговая аналитика загружается…</div></section>}>
          <IncomeTaxPanel calendar={trustedData} />
        </Suspense>
      )}
    </div>
  )
