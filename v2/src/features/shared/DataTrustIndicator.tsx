...
export function DataTrustIndicator({ trust, compact = false }: { trust: DataTrustSnapshot; compact?: boolean }) {
  ...
  return <span className={`data-trust is-${trust.status.toLowerCase()}`} role="img" aria-label={`${LABELS[trust.status]}. ${trust.shortReason}. Источник: ${source}. ${age}`} title={`${trust.shortReason} · ${source} · ${age}`}>
