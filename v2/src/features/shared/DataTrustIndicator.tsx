...
export function DataTrustIndicator({ trust, compact = false }: { trust: DataTrustSnapshot; compact?: boolean }) {
  ...
  return <span className={`data-trust is-${trust.status.toLowerCase()}`} role="status" aria-label={`${LABELS[trust.status]}. ${trust.shortReason}. Источник: ${source}. ${age}`} title={`${trust.shortReason} · ${source} · ${age}`}>
    <b>{LABELS[trust.status]}</b>{!compact && <small>{source} · {age}</small>}
  </span>
}
