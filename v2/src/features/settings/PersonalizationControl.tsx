...
const WORKSPACES: Array<{ key: UiWorkspace; label: string }> = [{key:'board',label:'BOARD'},{key:'portfolio',label:'PORTFOLIO'},{key:'analytics',label:'ANALYTICS'},{key:'income',label:'INCOME'},{key:'goals',label:'GOAL'},{key:'dna',label:'DNA'}]
...
export function PersonalizationControl({ preferences, onChange, onReset }: Props) {
  ...
  return <details ref={detailsRef} className="qv-personalize">
    ...
    <section className="qv-personalize__panel" aria-label="Персонализация QVANIX">
      <header><div><span>QVANIX SHELL</span><strong>ПЕРСОНАЛИЗАЦИЯ</strong></div><button type="button" className="qv-personalize__close" onClick={close} aria-label="Закрыть персонализацию">← <span>НАЗАД</span></button></header>
      ...
      <div className="qv-personalize__section"><div className="qv-personalize__label"><span>МОЯ ПАНЕЛЬ</span><small>{preferences.pinnedModules.length}/6 · нажмите для pin/unpin</small></div><div className="qv-module-grid">{MODULES.map(item=>{const active=preferences.pinnedModules.includes(item.key);return <ChoiceButton key={item.key} active={active} label={item.label} note={item.group} disabled={!active&&preferences.pinnedModules.length>=6} onClick={()=>toggleModule(item.key)}/>})}</div></div>
