...
export function HistoryChart({ points }: Props) {
  ...
  const portfolioPath = linePath(chartPoints.map(p => p.portfolio))
  const imoexPath = linePath(chartPoints.map(p => p.imoex))
  const first = chartPoints[0]?.date
  const last = chartPoints.at(-1)?.date

  return (
    <div className="history-chart">
      <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Доходность портфеля и IMOEX">
        ...
        {imoexPath && <path d={imoexPath} className="history-line history-line--imoex" />}
      </svg>
      <div className="history-legend">
        <span><i className="legend-dot legend-dot--portfolio" />Портфель</span>
        <span><i className="legend-dot legend-dot--imoex" />IMOEX</span>
