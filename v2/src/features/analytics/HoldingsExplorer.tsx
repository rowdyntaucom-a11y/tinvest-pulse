...
export function HoldingsExplorer({ positions, onOpenAsset }: { positions: PositionSnapshot[]; onOpenAsset: (position: PositionSnapshot) => void }) {
  ...
  return <section className="panel holdings-explorer">
    <div className="panel-head"><div><span className="eyebrow">СОСТАВ ПОРТФЕЛЯ</span><h2>СТРУКТУРА</h2></div><small>{filtered.length} поз.</small></div>
    <div className="holdings-control-group"><small>АКТИВЫ</small><div className="holdings-controls" aria-label="Фильтры активов">{filters.map(([key,label]) => <button key={key} className={filter===key?'is-active':''} onClick={()=>setFilter(key)}>{label}</button>)}</div></div>
    <div className="holdings-control-group"><small>ПОКАЗАТЬ ПО</small><div className="holdings-controls" aria-label="Группировка активов">{dimensions.map(([key,label]) => <button key={key} className={dimension===key?'is-active':''} onClick={()=>setDimension(key)}>{label}</button>)}</div></div>
    {instrumentView&&<div className="holdings-toolbar"><div>{([['compact','КОМПАКТНО'],['return','РЕЗУЛЬТАТ'],['risk','КОНЦЕНТРАЦИЯ'],['fundamental','ПОКАЗАТЕЛИ']] as Array<[HoldingPreset,string]>).map(([key,label])=><button key={key} className={preset===key?'is-active':''} onClick={()=>setPreset(key)}>{label}</button>)}</div><select value={sort} onChange={event=>setSort(event.target.value as HoldingSort)} aria-label="Сортировка"><option value="weight">По доле</option><option value="value">По стоимости</option><option value="pnl">По P/L</option><option value="name">По названию</option></select></div>}
