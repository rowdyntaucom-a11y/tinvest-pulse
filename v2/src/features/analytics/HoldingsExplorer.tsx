...
const filters: Array<[AssetClassFilter, string]> = [['all','ВСЕ'],['shares','АКЦИИ'],['bonds','ОБЛИГАЦИИ'],['funds','ФОНДЫ'],['currency','ВАЛЮТА'],['futures','FUTURES'],['other','ДРУГОЕ']]
...
export function HoldingsExplorer({ positions, onOpenAsset }: { positions: PositionSnapshot[]; onOpenAsset: (position: PositionSnapshot) => void }) {
  ...
  const filtered = useMemo(() => filterAndSortHoldings(positions, filter, sort), [positions, filter, sort])
