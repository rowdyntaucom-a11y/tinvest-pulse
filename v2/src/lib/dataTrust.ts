...
export function evaluateDataTrust(input: DataTrustInput): DataTrustSnapshot {
  ...
  const expiredByAge = ageMs != null && input.staleAfterMs != null && ageMs > input.staleAfterMs
  const isStale = input.explicitStale === true || expiredByAge
  ...
  return {
    ...
    staleReason: status === 'STALE' ? input.staleReason ?? (expiredByAge ? `Возраст данных превышает ${input.staleAfterMs} мс` : 'Источник явно пометил данные как устаревшие') : null,
...
export function evaluateHistoryTrust(points: Array<{ date: string; portfolio?: number | null; imoex?: number | null }>, nowMs: number) {
  const valid = points.filter(point => /^\d{4}-\d{2}-\d{2}$/.test(point.date) && Number.isFinite(Date.parse(`${point.date}T00:00:00.000Z`)))
  const hasPortfolioObservation = (point: (typeof valid)[number]) => typeof point.portfolio === 'number' && Number.isFinite(point.portfolio)
  const hasBenchmarkObservation = (point: (typeof valid)[number]) => typeof point.imoex === 'number' && Number.isFinite(point.imoex)
  const observations = valid.filter(point => hasPortfolioObservation(point) || hasBenchmarkObservation(point))
  const portfolioPoints = observations.filter(hasPortfolioObservation)
  const benchmarkPoints = observations.filter(hasBenchmarkObservation)
  const pairedPoints = observations.filter(point => hasPortfolioObservation(point) && hasBenchmarkObservation(point))
  const latestPairedDate = pairedPoints.reduce<string | null>((latest, point) => latest == null || point.date > latest ? point.date : latest, null)
  const latestObservationDate = observations.reduce<string | null>((latest, point) => latest == null || point.date > latest ? point.date : latest, null)
  const missingSegments = observations.length - pairedPoints.length
  const latestPortfolioDate = portfolioPoints.reduce<string | null>((latest, point) => latest == null || point.date > latest ? point.date : latest, null)
  const trust = evaluateDataTrust({ sourceId: 'T_INVEST+IMOEX', sourceType: 'MARKET', sourceState: observations.length ? 'LIVE' : 'UNAVAILABLE',
    sourceTimestamp: latestObservationDate, staleAfterMs: 7 * 86_400_000, coverage: missingSegments ? 'PARTIAL' : observations.length >= 2 ? 'COMPLETE' : 'INSUFFICIENT',
    hasData: observations.length > 0, partialReason: missingSegments ? `${missingSegments} наблюдений без подтверждённой пары` : 'Недостаточно истории', nowMs })
  const portfolioTrust = evaluateDataTrust({ sourceId: 'T_INVEST_HISTORY', sourceType: 'BROKER', sourceState: portfolioPoints.length ? 'LIVE' : 'UNAVAILABLE',
    sourceTimestamp: latestPortfolioDate, staleAfterMs: 7 * 86_400_000, coverage: portfolioPoints.length < observations.length ? 'PARTIAL' : portfolioPoints.length >= 2 ? 'COMPLETE' : 'INSUFFICIENT',
    hasData: portfolioPoints.length > 0, partialReason: 'История портфеля покрыта не полностью', nowMs })
  return { trust, portfolioTrust, portfolioPoints: portfolioPoints.length, benchmarkPoints: benchmarkPoints.length, pairedPoints: pairedPoints.length, latestObservationDate, latestPairedDate, missingSegments }
}

export function evaluatePayoutTrust(input: { loading?: boolean; available: boolean; stale: boolean; generatedAt: string | null; eligibleAssets: number; resolvedAssets: number; coverageRatio: number; scheduleComplete: boolean }, nowMs: number) {
  const complete = input.resolvedAssets === input.eligibleAssets && input.coverageRatio === 1 && input.scheduleComplete
  const coverage: DataCoverage = complete ? 'COMPLETE' : input.available ? 'PARTIAL' : 'UNKNOWN'
  return evaluateDataTrust({ sourceId: 'T_INVEST_PAYOUTS', sourceType: 'PAYOUT', sourceState: input.loading ? 'LOADING' : input.available ? 'LIVE' : 'UNAVAILABLE',
    fetchedAt: input.generatedAt, staleAfterMs: 24 * 60 * 60_000, explicitStale: input.stale, staleReason: 'Снимок выплат помечен источником как устаревший', coverage, hasData: input.available,
    partialReason: 'Не все активы имеют подтверждённые данные о выплатах', nowMs })
}

export function evaluateAssetHistoryTrust(input: { available: boolean; source: string | null; latestPointAt: string | null; requested: number; availableSeries: number; points: number }, nowMs: number) {
  const coverage: DataCoverage = input.availableSeries < input.requested ? 'PARTIAL' : input.points >= 2 ? 'COMPLETE' : 'INSUFFICIENT'
  return evaluateDataTrust({ sourceId: input.source, sourceType: 'MARKET', sourceState: input.available ? 'LIVE' : 'UNAVAILABLE', sourceTimestamp: input.latestPointAt,
...
export function ownsLatestRequest(sequence: number, currentSequence: number, active = true) {
  return active && sequence === currentSequence
}

export function allowsConfirmedEmptyPortfolio(trust: DataTrustSnapshot, positions: number) {
  return positions === 0 && trust.status === 'LIVE' && trust.coverage === 'COMPLETE'
}
