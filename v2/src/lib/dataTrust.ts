...
export function evaluatePayoutTrust(input: { loading?: boolean; available: boolean; stale: boolean; generatedAt: string | null; eligibleAssets: number; resolvedAssets: number; scheduleComplete: boolean }, nowMs: number) {
  const complete = input.eligibleAssets > 0 && input.resolvedAssets === input.eligibleAssets && input.scheduleComplete
