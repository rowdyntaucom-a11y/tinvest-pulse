...
export function evaluateHistoryTrust(points: Array<{ date: string; portfolio?: number | null; imoex?: number | null }>, nowMs: number) {
  ...
  const latestObservationDate = valid.reduce<string | null>((latest, point) => latest == null || point.date > latest ? point.date : latest, null)
  ...
  const trust = evaluateDataTrust({ sourceId: 'T_INVEST+IMOEX', sourceType: 'MARKET', sourceState: valid.length ? 'LIVE' : 'UNAVAILABLE',
    sourceTimestamp: latestObservationDate, staleAfterMs: 7 * 86_400_000, coverage: missingSegments ? 'PARTIAL' : valid.length >= 2 ? 'COMPLETE' : 'INSUFFICIENT',
  ...
  const portfolioTrust = evaluateDataTrust({ sourceId: 'T_INVEST_HISTORY', sourceType: 'BROKER', sourceState: valid.length ? 'LIVE' : 'UNAVAILABLE',
    sourceTimestamp: latestPortfolioDate, staleAfterMs: 7 * 86_400_000, coverage: portfolioPoints.length < valid.length ? 'PARTIAL' : portfolioPoints.length >= 2 ? 'COMPLETE' : 'INSUFFICIENT',
    hasData: portfolioPoints.length > 0, partialReason: 'История портфеля покрыта не полностью', nowMs })
  return { trust, portfolioTrust, portfolioPoints: portfolioPoints.length, benchmarkPoints: benchmarkPoints.length, pairedPoints: pairedPoints.length, latestPairedDate, missingSegments }
}
...
export function allowsConfirmedEmptyPortfolio(trust: DataTrustSnapshot, positions: number) {
  return positions === 0 && trust.status === 'LIVE' && trust.verifiedLive && trust.coverage === 'COMPLETE'
}
