...
- Added a private backend bridge `assetUid -> current portfolio exact UID check -> official InstrumentsService/GetAssetFundamentals`. The token remains server-only. Invalid, foreign, ambiguous and unsupported identity fails closed. Exact response identity is required and cached privately.
