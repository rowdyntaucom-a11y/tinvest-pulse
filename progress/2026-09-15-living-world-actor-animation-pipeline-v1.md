...
`reviewed local manifest → validated atlas document → isolated per-role load → animation selection → choreography actionProgress → deterministic frame playback → future Pixi sprite binding`
...
CI **#610** on head `b9b2f4c80910bd98041bcea501f76d37b984404f`: **fully green**.

Passed:
- dependency security gates;
- TypeScript + Vite production build;
- full `test:core`, including the new actor animation pipeline regressions;
...
No bundle/security/test threshold was weakened.

## Next safe step

Wire only successfully loaded reviewed actor packs into the deferred Pixi runtime on a per-role basis, while preserving procedural fallback for every missing/invalid role. Actual visual replacement remains gated on reviewed art assets and should not be faked with current geometry.
