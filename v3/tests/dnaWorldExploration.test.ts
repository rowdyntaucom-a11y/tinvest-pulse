...
assert.match(stage,/data-world-exploration="v2"/);assert.match(stage,/aria-label="Исследовать точки живого мира"/);assert.match(stage,/aria-pressed=\{landmark===item\.id\}/);assert.match(stage,/aria-expanded=\{landmark===item\.id\}/);assert.match(stage,/aria-controls="v3-world-landmark-detail"/);assert.match(stage,/id="v3-world-landmark-detail"/);assert.match(stage,/role="status"/);assert.match(stage,/aria-live="polite"/);assert.match(stage,/setLandmark\(null\)/);assert.ok((stage.match(/width:44,height:44/g)??[]).length>=2);assert.match(stage,/data-world-landmark-edge/);assert.match(stage,/projectWorldPointToScreen/);assert.match(stage,/clampWorldMarkerToViewport/);assert.match(stage,/focus=\{focus\}/);
