...
function Preview({kind,id}:{kind:GateKind;id:string}){
 if(kind==="assets")return <section id={id} className="nord-gate__preview nord-gate__preview--assets" aria-label="Структура после подтверждения данных">
 ...
 if(kind==="analysis")return <section id={id} className="nord-gate__preview nord-gate__preview--analysis" aria-label="Аналитика после подтверждения данных">
 ...
 return <section id={id} className="nord-gate__preview nord-gate__preview--income" aria-label="Доход после подтверждения данных">
...
export function NordTrustGate({kind,onRefresh,refreshing=false}:{kind:GateKind;onRefresh?:()=>void|Promise<void>;refreshing?:boolean}){
 const x=COPY[kind],previewId="nord-gate-preview-"+kind;
 return <main className={"nord-gate nord-gate--"+kind} aria-label={x.title}>
  <section className="nord-gate__world" aria-hidden="true">
   ...
   <NordScrollCue targetId={previewId} label="НИЖЕ · ПРИБОРЫ" placement="scene"/>
  </section>
  <Preview kind={kind} id={previewId}/>
  <section className="nord-gate__answer">
   ...
   <NordScrollCue targetId="nord-gate-route" label="ДАЛЬШЕ · МАРШРУТ" placement="inline"/>
  </section>
