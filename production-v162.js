...
if(!src.includes(marker))throw new Error('v16.2: v15.8 core compile marker changed');
...
const assetHistoryCode=[
 ...
 "    const candidates=(Array.isArray(portfolio?.positions)?portfolio.positions:[])",
 "      .map(p=>({raw:p,instrumentId:p?.instrumentUid||p?.figi||null,figi:p?.figi||null,instrumentType:p?.instrumentType||''}))",
 "      .filter(x=>x.instrumentId);",
 "    const valued=[];",
 "    for(let i=0;i<candidates.length;i+=3){",
 "      const chunk=candidates.slice(i,i+3);",
 "      const rows=await Promise.all(chunk.map(async item=>{",
 "        let meta=null;",
 "        if(String(item.instrumentType||'').toUpperCase().includes('BOND')&&item.figi){try{meta=await getInstrumentMeta(item.figi,item.instrumentType)}catch(metaError){}}",
 "        const value=assetHistoryPositionMarketValue(item.raw,meta);",
 ...
 "          if(!meta&&item.figi){try{meta=await getInstrumentMeta(item.figi,item.instrumentType)}catch(metaError){}}",
 ...
 "    return res.status(502).json({version:'1.1',available:false,series:[],error:'asset history unavailable'});",
 "  }",
 "});",
 ""
].join('\n');
