...
> **Priority update — 2026-09-24:** visual development is now deliberately serial. Finish **Samurai first** before materially developing another shell. The current Samurai hero artwork is approved and must be kept; its Home first-screen composition is also approved and should not be recomposed without new explicit user direction. Remaining Samurai work is atmosphere, distinctive motion, tactile interaction language, and stronger secondary-workspace identity. After Samurai is accepted, develop other shells **one at a time**, beginning each with its own artwork direction before interface/animation work. Do not spread visual effort across all shells in parallel.\n\n> **Priority update — 2026-09-23:** after real-device review, further Living World/DNA scene work is paused. The active presentation priority is the main workspaces and six complete, readable shell design systems. A shell must change interface material, geometry, typography, hierarchy and chrome—not merely background artwork or hue. DNA remains preserved as a first-class destination, but is not the current visual-development epic.
