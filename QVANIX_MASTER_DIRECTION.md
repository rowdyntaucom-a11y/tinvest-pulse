...
> **FINANCIAL CORE SPRINT — 2026-09-25 evening (LATEST USER DECISION):** freeze DNA / Living World indefinitely and treat it as a later optional module. Until Monday, focus on finishing deterministic financial functions, math, widgets, charts, module grouping and professional read-only analytics. PC is first-class alongside phone/tablet; Monday is the composition/widget-priority checkpoint. Multi-user registration + user-owned read-only broker connections remain mandatory architecture. QVANIX may support futures/options/order-book/scalping-oriented analytics when verified data exists, but must never expose order entry, execution, auto-trading or broader-than-needed broker scopes. Canonical sprint detail: `QVANIX_FINANCIAL_CORE_SPRINT.md`.

> **REFERENCE-SHELL LOCK — 2026-09-25 (latest user decision):** implement the full Snowball+ functional breadth **first in Samurai**. Samurai is now the reference product shell for ordinary financial workspaces. Do not fan out new functional modules across Cosmos/NORD/other shells until the Samurai implementation is complete enough for real-device acceptance. Financial engines, data contracts and deterministic calculations must remain shared/shell-neutral; only presentation, chapter composition and shell-specific motion belong to Samurai. After a Samurai feature set is accepted, transfer the same shared capability into other shells with their own visual language rather than reimplementing the math. This latest lock overrides older shell-rotation notes that called Samurai “done” or made Cosmos the active implementation target.\n\n> **HARD PRIORITY LOCK — 2026-09-24 (latest user decision):** **Living World / DNA WORLD is frozen and is now the lowest-priority product area.** Do not spend development time on DNA art, scene depth, animations, gamification, world-state mechanics, characters, events or new DNA UX while the ordinary investment workspaces are incomplete. Existing DNA may remain accessible, but only blocking regressions that break the rest of the application justify touching it. The serial product order is now: **Assets Depth → Analysis Depth → Income Depth → Goal/main-tab completion → cross-shell readability and real-device polish → only then return to DNA WORLD.** Main financial tabs and their shared deterministic depth have priority over gamification.

> **Product-depth priority — 2026-09-24 (current):** after the Samurai/Cosmos first-screen architecture was established, the next major stage is **Assets Depth v1**. The shell stays atmospheric above; a visible cue leads into one shared deep portfolio workspace below. Asset composition, concentration, broker P/L attribution, metadata coverage, bond diagnostics and canonical asset drill-down must use common deterministic models across every shell. Shells may change composition, material and motion, but must not fork financial logic. After Assets Depth is stable, apply the same architecture to Analysis Depth and Income Depth.

> **Deep-workspace / Snowball+ rule — 2026-09-25 (latest user reminder):** every main tab in every shell is a vertically extensible workspace, not a one-screen dashboard. The first viewport must answer quickly and preserve shell atmosphere; below it, additional functional chapters may continue as far as needed. A visible, non-obstructive down cue should hand the user from the scene into the next chapter without requiring a blind finger drag. Cues must live in art-safe zones or normal document flow — never float over finance controls. QVANIX targets **at least Snowball-level functional breadth and then exceeds it with transparent deterministic analytics, verified data, deeper diagnostics and QVANIX-specific interaction/visual language**. Do not cap a tab merely to keep it visually short; only Pulse/Screenshot mode remains intentionally one-screen.

> **Snowball mobile inventory — 2026-09-25:** user recording `1000031720.mp4` expands the parity benchmark beyond analytics alone. Required breadth now explicitly includes portfolio/event hygiene, operations, payout calendar, broad-market dividend calendar, category/currency views, corporate actions, benchmark periods, fundamental portfolio metrics when source-verified, dedicated bond analytics, rebalancing, Portfolio Laboratory/backtest-like strategy comparison, fallen-asset/technical discovery, screeners and later social/public/account surfaces. Detailed observed inventory: `progress/2026-09-25-snowball-mobile-recording-inventory.md`. Snowball remains the minimum breadth benchmark; QVANIX must not copy its UI or weaken fail-closed methodology to claim parity.

> **Navigation continuity correction — 2026-09-24:** Samsung recording `1000031564.mp4` exposed that internal workspace scroll can survive a primary-tab switch even when `window.scrollTo(0,0)` runs. Primary navigation must synchronously reset the active workspace scroll owner before paint, otherwise verification/deep content can flash over the shell artwork during tab changes.

> **Samurai motion/depth update — 2026-09-24:** Samsung recording `1000031563.mp4` confirms the art-safe first view is working, but the transition into verification depth must be authored too. For Samurai secondary tabs, the Ronin artwork belongs to the first viewport only; a light swipe or down cue should snap into a distinct dark verification chamber rather than slide old route cards across the artwork. Assets / Analysis / Income keep separate Formation / Tactical / Treasury chamber identities.
...
- DNA may remain reachable as a preserved product destination, but its development is frozen and lowest priority until the main financial workspaces are complete; do not spend design/engineering time optimizing DNA merely to preserve equal prominence;
