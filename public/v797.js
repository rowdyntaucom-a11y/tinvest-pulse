(()=>{
 ...
 function cleanDuplicate(el){
  const raw=el.textContent||'';
  el.textContent=raw.replace(/\s*\([+−-]?\d+(?:[.,]\d+)?%\)/g,'');
 }
 function explain(el){
  if(!el)return;cleanDuplicate(el);
  const raw=(el.textContent||'').trim(),m=raw.match(/([+−-]?\d+(?:[.,]\d+)?)\s*п\.п\./i);if(!m)return;
  const n=num(m[1].replace('−','-'));if(!Number.isFinite(n))return;
  let h=el.nextElementSibling;if(!h||!h.classList.contains('ppExplain')){h=document.createElement('small');h.className='ppExplain';el.insertAdjacentElement('afterend',h)}
  h.textContent=context(el,n);el.dataset.ppExplain=String(n);
 }
 function scan(){document.querySelectorAll('body *').forEach(el=>{if(el.children.length===0&&/п\.п\./i.test(el.textContent||''))explain(el)})}
 const css=document.createElement('style');css.textContent='.ppExplain{display:block;margin-top:3px;font-size:6px;line-height:1.2;letter-spacing:.15px;color:#81918d;font-weight:600;text-transform:none}.mini .ppExplain{font-size:6px}.w77controls .ppExplain{display:block;margin:3px 0 0;text-align:right}.w77grid .ppExplain,.stGrid .ppExplain{font-size:5px}';document.head.appendChild(css);
