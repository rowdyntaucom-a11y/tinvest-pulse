...
const transactionMarkerRouteCode=[
 ...
 "      const brokerOperationId=typeof op?.id==='string'&&op.id.trim()?op.id.trim():null;",
 ...
 "      if(!qvanixTransactionMarkerTypes.has(type)||!brokerOperationId||!date||(!figi&&!instrumentUid))continue;",
 "      const operationId=crypto.createHash('sha256').update(brokerOperationId).digest('hex').slice(0,24);",
 ...
 "    const payload={version:'1.0',available:true,markers,source:'T-Bank GetOperationsByCursor',asOf:new Date(now).toISOString(),identitySemantics:'hashed_broker_operation_id_snapshot_only',priceIncluded:false,cacheSeconds:900};",
